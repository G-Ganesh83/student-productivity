...
function Productivity() {
  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        ...
        const message = getTaskApiErrorMessage(err, "Unable to load tasks.");
        setError(message);
        const id = getNextId();
  ...
  const saveTask = async () => {
    ...
    try {
      ...
      console.log(err.response);
      const message = getTaskApiErrorMessage(err, "Unable to save task.");
      setError(message);
  ...
  const handleDeleteTask = async (id) => {
    ...
    try {
      ...
      console.log(err.response);
      const message = getTaskApiErrorMessage(err, "Unable to delete task.");
      setError(message);
      addToast(message, "error");
    } finally {
  ...
  const toggleStatus = async (id) => {
    try {
      ...
      console.log(err.response);
      const message = getTaskApiErrorMessage(err, "Unable to update task status.");
      setError(message);
      addToast(message, "error");
    } finally {
      ...
      {loading && tasks.length === 0 && (
        <Card variant="subtle" padding="lg">
          <div className="text-center py-10">
            <div className="w-8 h-8 mx-auto mb-3 rounded-full border-2 border-slate-200 border-t-brand-500 animate-spin" />
      ...
      {!loading && total === 0 ? (
        <Card variant="brand" padding="lg">
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-2">No tasks yet. Create your first task 🚀</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filteredTasks.map((task) => {
            ...
            return (
              <Card key={task._id} variant="default" padding="none" className="overflow-hidden group">
                {/* Priority bar */}
            ...
            <Button
                      ...
                      disabled={isSavingTask || isDeletingTaskId === task._id || isTogglingTaskId === task._id}
                      className="flex-1"
                    >
                      {isTogglingTaskId === task._id ? "Saving..." : isDone ? "↩ Undo" : "✓ Done"}
                    </Button>
                    <Button
                      variant="secondary"
                      size="xs"
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="secondary" onClick={closeModal} disabled={isSavingTask}>Cancel</Button>
